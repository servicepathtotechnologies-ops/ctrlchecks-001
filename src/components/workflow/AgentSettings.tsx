...
export default function AgentSettings({ workflowId }: AgentSettingsProps) {
  ...
  const loadConfig = async () => {
    ...
    try {
      ...
      if (data) {
        setWorkflowType(data.workflow_type || 'automation');
        if (data.agent_config) {
          setConfig({
            goal: (data.agent_config as any).goal || '',
            maxIterations: (data.agent_config as any).maxIterations || 10,
            reasoningModel: (data.agent_config as any).reasoningModel || 'gpt-4o',
            actionModel: (data.agent_config as any).actionModel || 'gpt-4o',
            memoryEnabled: (data.agent_config as any).memoryEnabled !== false,
            temperature: (data.agent_config as any).temperature || 0.3,
          });
        }
      }
    } catch (error) {
