...
export default function DebugPanel({ onClose }: DebugPanelProps) {
  ...
  const handleRunNode = useCallback(async () => {
    ...
    try {
      ...
      // Execute single node
      const { data, error } = await supabase.functions.invoke('execute-node', {
        body: {
          runId, // UUID v4 for debug execution
          nodeId: debugNodeId,
          nodeType: debugNode.data.type,
          config: nodeConfig,
          input: inputData, // Changed from inputData to input to match API
          workflowId,
        },
      });
